...
不需要把自动生成的每一个空间分区都分类完毕。工作区按可见表面显示 `已标注/总数` 和百分比；封闭在模型内部的
Unknown 不影响上色。可见覆盖不足时会进入质量审阅提示，但不会仅因“未分类分区数量”禁止编译。人工基准色是
主体侧面的实际颜色锚点，派生明暗优先使用它所在的 RA2 连续索引色阶。切换基准色或技法会清除旧候选并自动
返回语义/原始 3D，需再次点击“编译上色预览”。

画笔只处理当前可见的外露表面；每个可见面会精确映射回所属体素，不再用屏幕附近的体素猜测。空白处左键
会显示“未命中模型表面”且不修改蒙版。旋转模型即可接触另一侧。它不修改几何、不会自动保存，也不会写 VXL/HVA。
拖动过程中黄色表示待提交绘制路径、红色表示待提交擦除路径；它们只显示命中采样点。中央预览工具栏中的
`分类预览：部件 / 材质` 可随时切换当前审阅配色和图例；点击后会返回语义 3D，但不会改变当前五阶段工作步骤。
这些标注色不等于游戏内色板颜色。
