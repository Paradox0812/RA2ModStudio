...
internal sealed class Ra2FieldDiagnosticService
{
        ...
        int maximumResultItems = int.MaxValue)
    {
        ...
        for (int index = 0; index < semanticModel.KeyValues.Count; index++)
        {
            if (index % Ra2DocumentDiagnosticService.CancellationCheckInterval == 0)
                cancellationToken.ThrowIfCancellationRequested();

            Ra2KeyValueSymbol keyValue = semanticModel.KeyValues[index];
            ...
            AddTrustDiagnostics(snapshot, keyValue, definition!, issues, maximumResultItems);
            AddValueDiagnostics(snapshot, keyValue, definition!, issues, maximumResultItems);
        }
        ...
        int maximumResultItems)
    {
        ...
        switch (metadata.ValueKind)
        {
            case Ra2FieldValueKind.Boolean:
                AddBooleanDiagnostic(snapshot, keyValue, metadata, value, issues, maximumResultItems);
                break;
            case Ra2FieldValueKind.Enum:
                AddEnumDiagnostic(snapshot, keyValue, metadata, value, issues, maximumResultItems);
                break;
            case Ra2FieldValueKind.EnumList:
                AddEnumListDiagnostic(snapshot, keyValue, metadata, value, issues, maximumResultItems);
                break;
            case Ra2FieldValueKind.Integer:
                AddIntegerDiagnostic(snapshot, keyValue, value, issues, maximumResultItems);
                break;
            case Ra2FieldValueKind.Float:
                AddFloatDiagnostic(snapshot, keyValue, value, issues, maximumResultItems);
                break;
        }
    }
